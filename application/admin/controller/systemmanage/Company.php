...
class Company extends Index
{
    ...
    public function get_company_list() {
        ...
        $listSize = count($companyList);
        $resData = array_slice($companyList,$limitStart,$_POST['pSize']);//此处由于需要分页，所以不是返回所有结果
        //封装成gridManager需要的格式
        $result['data'] = $resData;
                ...
                if($resObjectid){
                    ...
                    try{
                        ...
                        $createDB3 = "CREATE TABLE `ms_data_history_".$resObjectid."` (
                                        ...
                                        `uid` int(10) unsigned NOT NULL COMMENT '终端id标识',
                                        ...
                                        `aptime` datetime DEFAULT NULL COMMENT '基站处理时间aptime',
     ...
     * @return int
     */
    public function check_companyname_duplicate() {
        if(!empty($_POST)){
            $global_company = db('global_company');
     ...
     * @return int
     */
    public function check_companycode_duplicate() {
        if(!empty($_POST)){
            $global_company = db('global_company');
            $Info = $global_company ->where("companycode = '{$_POST["companycode"]}' and flag <> 3") ->find();
            //校验用户名,需要封装好的bootstrapValidator remote校验需要的验证格式，必须是json
            if($Info){
                $remoteJson["valid"] = false;
            }else{
